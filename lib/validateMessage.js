...
exports.validateMessage = function validateMessage(raw, sourceFile) {
  ...
  if (!match) {
    ...
    var SCOPE_PATTERN = new RegExp(config.scopePattern);
    var SCOPE_PATTERN_ERROR_MSG = config.scopePatternErrorMsg || 'scope does not match scope pattern!';
    ...
    if (!SCOPE_PATTERN.exec(scope)) {
      error(SCOPE_PATTERN_ERROR_MSG);
      isValid = false;
    }
